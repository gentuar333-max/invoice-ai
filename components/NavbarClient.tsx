...
const tabs = [
  ...
  { href: "/dashboard?tab=contrats", label: "Contrats" },
];
    ...
    <>
      <style>{`
        ...
        .ia-header {
          ...
          top: 0;
          z-index: 100;
          box-shadow: 0 2px 20px rgba(107,74,42,0.18);
        }
        .ia-header-top {
          display: flex;
          align-items: center;
        ...
        .ia-nav-tab.active {
          color: var(--creme);
          border-bottom-color: var(--cafe-pale);
        }
        .ia-nav-tab:hover:not(.active) {
          color: #e0cdb8;
        }
      `}</style>
      ...
      <header className="ia-header">
        <div className="ia-header-top">
          <a href="/dashboard" className="ia-logo">
            Invoice<span>Agent</span>
          </a>
          {isLoggedIn ? (
            <button className="ia-avatar" onClick={handleLogout} title="Deconnexion">
              {initials}
            </button>
          ) : (
            <a href="/auth/login" className="ia-avatar" style={{ textDecoration: "none" }}>
              IA
            </a>
          )}
        </div>
        <nav className="ia-nav-tabs">
          {tabs.map(tab => (
            <a
              key={tab.href}
              href={(tab as any).tab === "contrats" ? "/dashboard?tab=contrats" : tab.href}
              className={`ia-nav-tab${activeTab === ((tab as any).tab || tab.href) ? " active" : ""}`}
            >
              {tab.label}
            </a>
          ))}
