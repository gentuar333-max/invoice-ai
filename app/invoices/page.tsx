...
export default function InvoicesPage() {
  ...
  function checkDuplicate(data: InvoiceData) {
    const duplicate = history.find((inv) => {
      const sameVendor =
        inv.data.vendor_name?.toLowerCase() ===
        data.vendor_name?.toLowerCase();
      const sameNumber =
        data.invoice_number &&
        inv.data.invoice_number === data.invoice_number;
      const sameTotal =
        inv.data.total_amount === data.total_amount;
      const sameDate =
        inv.data.invoice_date === data.invoice_date;
    ...
    if (duplicate) {
      setDuplicateWarning(
        `⚠️ Facture similaire déjà enregistrée — ${duplicate.data.vendor_name}, ${duplicate.data.invoice_date}, ${duplicate.data.total_amount}€`
      );
    }
  }
  ...
  function handleExportCSV() {
    ...
    const rows = history.map((inv) => [
      inv.data.vendor_name || "",
      inv.data.invoice_number || "",
      inv.data.invoice_date || "",
      inv.data.due_date || "",
      inv.data.currency || "EUR",
      inv.data.subtotal ?? "",
      inv.data.tax_amount ?? "",
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Importez, vérifiez et enregistrez vos factures
          </p>
        </div>
        {history.length > 0 && (
          <button
      ...
      {!extracted && (
        <div
          ...
          onDrop={handleDrop}
          className={`border-2 border-dashed rounded-xl p-12 text-center transition-all mb-4 ${
            dragging
      ...
      {file && !extracted && (
        <button
          ...
          disabled={loading}
          className={`w-full py-3 rounded-lg text-white font-semibold text-sm transition mb-4 ${
            loading
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
          ...
          <div className="bg-blue-600 px-6 py-4">
            <p className="text-blue-200 text-xs uppercase tracking-wider mb-1">
              Vérifiez et corrigez avant d'enregistrer
            </p>
            <h2 className="text-white text-xl font-bold">
              {edited.vendor_name || "Fournisseur inconnu"}
            </h2>
          </div>
          ...
          <div className="p-6 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <EditableField
                label="Sous-total"
                value={String(edited.subtotal ?? "")}
                onChange={(v) => updateField("subtotal", parseFloat(v) || null)}
                type="number"
              />
              <EditableField
                label="TVA"
                value={String(edited.tax_amount ?? "")}
                onChange={(v) => updateField("tax_amount", parseFloat(v) || null)}
                type="number"
              />
              <EditableField
                label="Total TTC"
                value={String(edited.total_amount ?? "")}
                onChange={(v) => updateField("total_amount", parseFloat(v) || null)}
                type="number"
              />
            </div>
            ...
            {edited.line_items && edited.line_items.length > 0 && (
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-3">
                  Lignes · {edited.line_items.length} articles
                </p>
                        ...
                        <tr key={i} className="border-t border-gray-50">
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-full text-sm text-gray-700 border-0 focus:outline-none bg-transparent"
                            />
                          </td>
                          <td className="px-3 py-2 text-center">
                            <input
                              ...
                              value={item.quantity ?? ""}
                              onChange={(e) =>
                                updateLineItem(i, "quantity", parseFloat(e.target.value) || null)
                              }
                              className="w-full text-sm text-center text-gray-500 border-0 focus:outline-none bg-transparent"
                            />
                          </td>
                          <td className="px-3 py-2 text-right">
                            <input
                              ...
                              value={item.total ?? ""}
                              onChange={(e) =>
                                updateLineItem(i, "total", parseFloat(e.target.value) || null)
                              }
                              className={`w-full text-sm text-right border-0 focus:outline-none bg-transparent font-medium ${
            ...
            {edited.missing_fields && edited.missing_fields.length > 0 && (
              <div className="bg-amber-50 border border-amber-100 rounded-lg px-4 py-3">
                <p className="text-xs text-amber-600 font-medium mb-1">
                  ⚠️ Champs non trouvés :
                </p>
                <p className="text-xs text-amber-500">
                  {edited.missing_fields.join(", ")}
                </p>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                onClick={handleReset}
                className="border border-gray-200 text-gray-500 hover:bg-gray-50 font-medium py-3 px-5 rounded-lg text-sm transition"
              >
                Annuler
              </button>
            </div>
      ...
      {saved && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-6 mb-6 text-center">
          ...
          <button
            onClick={handleReset}
            className="bg-white border border-green-200 text-green-700 px-5 py-2 rounded-lg text-sm font-medium hover:bg-green-50 transition"
          >
            ← Nouvelle facture
          </button>
        </div>
      ...
      {history.length > 0 && (
        <div>
          ...
          {history.map((inv) => (
            <div
              key={inv.id}
              className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg px-4 py-3 mb-2 hover:border-gray-300 transition"
            >
