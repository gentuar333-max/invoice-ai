...
const schemaFAQ = {
  ...
  mainEntity: [
    { '@type': 'Question', name: "Quelles erreurs de facture empechent de recuperer la TVA ?", acceptedAnswer: { '@type': 'Answer', text: "Les 5 erreurs qui bloquent la deduction : numero de TVA intracommunautaire invalide, mentions obligatoires absentes, taux de TVA incorrect, erreur de calcul HT/TVA/TTC, et facture au nom d'une entite differente du declarant. InvoiceAgent les detecte en 5 secondes." } },
    { '@type': 'Question', name: "Combien de TVA les PME perdent-elles a cause d'erreurs de factures ?", acceptedAnswer: { '@type': 'Answer', text: "En moyenne 6 200 euros par an de TVA non recuperable a cause d'erreurs formelles sur les factures fournisseurs. Cela represente environ 3% du montant total de TVA deductible pour une PME francaise." } },
    { '@type': 'Question', name: "Comment verifier qu'une facture permet la deduction de TVA ?", acceptedAnswer: { '@type': 'Answer', text: "Verifiez 5 points : numero TVA valide sur VIES, 12 mentions obligatoires presentes, taux correct selon la nature des biens ou services, calculs HT/TVA/TTC exacts, facture au nom de votre entite juridique. InvoiceAgent verifie tout en 5 secondes." } },
    { '@type': 'Question', name: "InvoiceAgent verifie-t-il le numero de TVA sur VIES ?", acceptedAnswer: { '@type': 'Answer', text: "Oui. InvoiceAgent extrait le numero de TVA de chaque facture PDF et verifie sa validite sur la base europeenne VIES. Un numero invalide ou expire est signale avant comptabilisation." } },
  ],
};
            ...
            <div className="bg-white rounded-3xl shadow-2xl shadow-violet-600/20 p-6 border border-slate-100">
              ...
              <div className="space-y-2 mb-4">
                {[
                  ...
                  { point: 'Calcul TVA incorrect', detail: 'Base HT 4 200€ x 20% = 840€ mais facture indique 870€', status: 'Ecart 30€', color: 'amber' },
...
function Features() {
  const features = [
    ...
    { icon: Calculator, title: 'Recalcul integral HT / TVA / TTC', description: "L'IA recalcule chaque ligne de la facture : base HT multipliee par le taux donne le montant TVA attendu, puis verifie la coherence avec le total TTC. Les erreurs d'arrondi, les inversions de montants et les ecarts de calcul sont detectes au centime pres. Sur les factures multi-taux, ces erreurs sont particulierement frequentes.", color: 'from-orange-400 to-red-500' },
...
function Pricing() {
  const plans = [
    { name: 'Gratuit', price: '0', description: 'Pour tester', features: ['5 factures verifiees', 'Numero TVA verifie VIES', 'Mentions obligatoires controlees', 'Rapport conformite PDF'], cta: 'Commencer gratuitement', popular: false, href: `${BASE_URL}/auth/login` },
...
function FAQ() {
  const faqs = [
    { q: "Quelles erreurs de facture empechent de recuperer la TVA ?", a: "Les 5 erreurs qui bloquent la deduction de TVA lors d'un controle fiscal : 1) Numero de TVA intracommunautaire invalide ou expire — la verification VIES est indispensable, 2) Mentions obligatoires absentes comme l'adresse complete ou le SIRET, 3) Taux de TVA incorrect par rapport a la nature de la prestation facturee, 4) Erreur de calcul entre base HT, montant TVA et total TTC, 5) Facture emise au nom d'une entite juridique differente de celle qui deduit la TVA." },
    { q: "Combien de TVA les PME perdent-elles a cause d'erreurs de factures ?", a: "En moyenne, une PME francaise perd 6 200€ par an de TVA non recuperable a cause d'erreurs formelles sur les factures fournisseurs. Ce montant represente environ 3% de la TVA deductible totale. Les erreurs les plus couteuses sont les numeros de TVA invalides — la deduction est rejetee integralement — et les mentions manquantes qui rendent la facture contestable lors d'un controle." },
    { q: "Comment verifier qu'une facture permet la deduction de TVA ?", a: "Verifiez systematiquement 5 points avant de comptabiliser : 1) Le numero de TVA du fournisseur est valide sur le systeme europeen VIES, 2) Les 12 mentions obligatoires sont presentes et exactes, 3) Le taux de TVA correspond a la nature des biens ou services factures, 4) Les calculs HT multiplie par le taux egale TVA et HT plus TVA egale TTC sont corrects, 5) La facture est adressee a votre entite juridique exacte. InvoiceAgent verifie ces 5 points en 5 secondes par facture." },
    ...
    { q: "Que faire si une facture fournisseur comporte une erreur de TVA ?", a: "Ne comptabilisez jamais une facture non conforme. Demandez une facture rectificative au fournisseur avec les corrections necessaires avant de l'integrer en comptabilite. En cas de controle fiscal, toute TVA deduite sur une facture non conforme est rejetee — vous devrez la rembourser avec des penalites de retard de 0,2% par mois et une majoration de 10% a 40%. InvoiceAgent detecte ces erreurs en 5 secondes." },
  ];
