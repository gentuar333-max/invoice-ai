...
RETOURNE UNIQUEMENT ce JSON valide sans texte autour:
{
  "insights": [
    {
      "id": "insight_1",
      "type": "risk",
      "vendor": "nom du fournisseur ou General",
  ...
  "summary": {
    "total_at_risk": 1500.00,
    "total_savings_possible": 200.00,
    "health_score": 65
  }
}`;
    ...
    if (!jsonMatch) {
      return NextResponse.json({ success: false, error: "Erreur IA — réessayez" }, { status: 500 });
    }
  ...
  Object.entries(byVendor).forEach(([vendor, invList]) => {
    ...
    invList.forEach((inv) => {
      if (inv.invoice_date && inv.due_date) {
        const diff = Math.round((new Date(inv.due_date).getTime() - new Date(inv.invoice_date).getTime()) / 86400000);
    ...
    if (duplicates.length > 0) summary += `  DOUBLONS: ${duplicates.join(", ")}€\n`;
    summary += "\n";
  });
